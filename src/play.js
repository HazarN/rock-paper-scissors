...
const checkWinner = (player, ai) => {
  ...
  if (player === ai) {
    ...
    (player === object.rock && ai === object.scissors) ||
    (player === object.paper && ai === object.rock) ||
    (player === object.scissors && ai === object.paper)
  ) {
    return ['You Won!', 1];
  } else {
    return ['You Lost!', -1];
...
window.onload = () => {
  if (performance.navigation.type === 1) {
    const images = document.querySelectorAll('main > *');
    images.forEach(image => {
      image.classList.remove('anim-default');
    });
    window.location.href = 'index.html';
  }
};
...
returnE.onclick = () => {
  if (returnState) {
    ...
    document
      .querySelector('body')
      .classList.remove('win-background', 'lose-background', 'draw-background');
